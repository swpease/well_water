...
```{r}
input_data = all_data |> 
  select(frac_WW_CEISIN, pop_density, all_of(covnames)) |> 
  tidyr::drop_na()
```

# prcomp
```{r}
mod = prcomp(input_data[,3:9], scale. = TRUE)
```

```{r}
screeplot(mod, type = "lines")
```
...
```{r}
mod_w_popdens = prcomp(input_data[,2:9], scale. = TRUE)
```
...
```{r}
ggbiplot::ggbiplot(mod, varname.color = "red", )
```
