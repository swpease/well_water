...
# BPR
```{r}
mod_no_rural = PReMiuM::profRegr(
  covNames = covnames,
  # fixedEffectsNames = c("pop_density"), # Is this accounting for everything?
  outcome = "WW_pop_OUTCOME",
  outcomeT = "E_TOTPOP",
  data = input_data,
  output = "bpr_output/mod_no_rural",
...
```{r}
dissimmat_mod_no_rural = calcDissimilarityMatrix(mod_no_rural)
```

```{r}
optimal_clust_mod_no_rural = calcOptimalClustering(dissimmat_mod_no_rural)
```

```{r}
# input_data["cluster_mod_no_rural"] = as.factor(optimal_clust_mod_no_rural[["clustering"]])
```

```{r}
riskProfileObj_mod_no_rural <- calcAvgRiskAndProfile(optimal_clust_mod_no_rural)
```

```{r}
clusterOrderObj_mod_no_rural <- plotRiskProfile(riskProfileObj_mod_no_rural, "risk_profile_mod_no_rural.png")
```
