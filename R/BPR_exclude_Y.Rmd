...
# BPR
```{r}
mod_exclude_Y = PReMiuM::profRegr(
  covNames = covnames,
  fixedEffectsNames = c("pop_density"),
  outcome = "WW_pop_OUTCOME",
  outcomeT = "E_TOTPOP",
  data = input_data,
  output = "bpr_output/mod_exclude_Y",
...
```{r}
dissimmat_mod_exclude_Y = calcDissimilarityMatrix(mod_exclude_Y)
```

```{r}
optimal_clust_mod_exclude_Y = calcOptimalClustering(dissimmat_mod_exclude_Y)
```

```{r}
# input_data["cluster_mod_exclude_Y"] = as.factor(optimal_clust_mod_exclude_Y[["clustering"]])
```

```{r}
riskProfileObj_mod_exclude_Y <- calcAvgRiskAndProfile(optimal_clust_mod_exclude_Y)
```

```{r}
clusterOrderObj_mod_exclude_Y <- plotRiskProfile(riskProfileObj_mod_exclude_Y, "risk_profile_mod_exclude_Y.png")
```
